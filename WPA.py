...
def WPA_pipline():
    ...
    for it in range(0, max_iteration):
        ...
        # 初始化头狼
        L_Wolf_V = np.min(wolf_colony_V)  # 记录头狼的值
        L_Wolf_index = np.argmin(wolf_colony_V)  # 头狼在狼群(wol)f_colony_X)中下标位置
        L_Wolf_X = wolf_colony_X[L_Wolf_index]  # 记录头狼的变量信息
        ...
        while ~surrounded:
            ...
            for m in range(0, M_Wolf.shape[0]):
                ...
                while d_near < dd:
                    single_M_Wolf = single_M_Wolf + step_b * (L_Wolf_X - single_M_Wolf) / np.abs(
